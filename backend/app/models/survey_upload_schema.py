...
class EmployeeSurveyRow(BaseModel):
    ...
    frecuencia_uso_ia: Optional[int] = Field(None, ge=1, le=5, description="AI usage frequency")
    ...
    AT1_rendimiento: Optional[float] = Field(None, ge=1, le=7)
    AT2_facilita_aprendizaje: Optional[float] = Field(None, ge=1, le=7)
    AT3_facilidad_uso: Optional[float] = Field(None, ge=1, le=7)
    AT4_integracion_positiva: Optional[float] = Field(None, ge=1, le=7)
    
    # Efficacy scales (AE1-AE4)
    AE1_resolver_problemas: Optional[float] = Field(None, ge=1, le=7)
    AE2_confianza_digital: Optional[float] = Field(None, ge=1, le=7)
    AE3_uso_eficaz: Optional[float] = Field(None, ge=1, le=7)
    AE4_seguridad_aplicacion: Optional[float] = Field(None, ge=1, le=7)
    
    # Motivation scales (M1-M4)
    M1_estimulante: Optional[float] = Field(None, ge=1, le=7)
    M2_aumenta_interes: Optional[float] = Field(None, ge=1, le=7)
    M3_aporta_valor: Optional[float] = Field(None, ge=1, le=7)
    M4_mayor_esfuerzo: Optional[float] = Field(None, ge=1, le=7)
    
    # Engagement scales (E1-E4)
    E1_adaptacion: Optional[float] = Field(None, ge=1, le=7)
    E2_feedback_util: Optional[float] = Field(None, ge=1, le=7)
    E3_aplicable_trabajo: Optional[float] = Field(None, ge=1, le=7)
    E4_aprendizaje_ritmo: Optional[float] = Field(None, ge=1, le=7)
    
    # Development scales (D1-D4)
    D1_mejora_competencias: Optional[float] = Field(None, ge=1, le=7)
    D2_preparado_retos: Optional[float] = Field(None, ge=1, le=7)
    D3_amplia_habilidades: Optional[float] = Field(None, ge=1, le=7)
    D4_aprendizaje_autonomo: Optional[float] = Field(None, ge=1, le=7)
    
    # Confidence scales (C1-C4)
    C1_confio_sin_verificar: Optional[float] = Field(None, ge=1, le=7)
    C2_dificil_sin_ia: Optional[float] = Field(None, ge=1, le=7)
    C3_pensamiento_critico: Optional[float] = Field(None, ge=1, le=7)
    C4_reflexiono_calidad: Optional[float] = Field(None, ge=1, le=7)
